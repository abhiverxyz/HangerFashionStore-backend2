...
/** Single-item analysis: same taxonomy as extraction (category_lvl1, type) for Get Ready / Concierge. */
...
- color: string (primary color, e.g. Blue, Black, White)
- description: string (one short phrase describing the item, e.g. "Blue casual shirt", "Denim skinny jeans")`;

/** Vision prompt that asks for bbox per item (normalized 0-1) for cropping. */
 ...
 * Uses same type/category taxonomy as extractFromLook for consistent use in Get Ready / Concierge.
 * @param {string|Buffer} imageUrlOrBuffer - Image as URL (resolved if our storage) or buffer
 * @returns {Promise<{ category: string | null, color: string | null, tags: string | null, type?: string | null }>}
 */
export async function analyzeItem(imageUrlOrBuffer) {
  ...
  const color = result?.color != null ? String(result.color).trim() || null : null;
